	...
	<div class="row action-navbar" id="action-navbar">
		<div class="col-md-12">
			...
			<a type='button' class="btn btn-danger btn-sm" href="{{admin_url()}}">
				...
				{{ __('ikpanel-blog::blog.categories.show.buttons.close') }}
			</a>
		</div>
	</div>
@endsection
...
@section('content')
	<div class="row">
		<div class="col-md-9">
			<div class="card">
				<div class="card-body">
					@component('ikpanel-blog::categories.table', ['categories' => $categories])
					@endcomponent
				</div>
			</div>
		</div>
						...
						<div class="col-md-12">
							<div class="form-group form-group-default ">
								<label for="search-filter">
									{{ __('ikpanel-blog::blog.categories.show.search') }}
								</label>
						...
						<div class="col-md-12">
							<div class="form-group form-group-default form-group-default-select2">
								<label for="status-filter">
									{{ __('ikpanel-blog::blog.categories.show.filterLabel') }}
								</label>
								<select id="status-filter" class="full-width select2-hidden-accessible" tabindex="-1"
								        aria-hidden="true"
								        autocomplete="off">
									<option value="ALL">
										{{ __('ikpanel-blog::blog.categories.show.filters.all') }}
									</option>
									<option value="ACTIVE" selected>
										{{ __('ikpanel-blog::blog.categories.show.filters.active') }}
									</option>
									<option value="DELETED">
										{{ __('ikpanel-blog::blog.categories.show.filters.deleted') }}
									</option>
								</select>
							</div>
						</div>
...
@section('final_script')
	...
	{!! script('ikpanel/modules/blog/js/category/show.js',true) !!}
@endsection