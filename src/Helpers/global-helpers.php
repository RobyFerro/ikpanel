...
function admin_url($url = '')
{
    if (App::runningInConsole()) {
        ...
        $protocol = 'http'.(Request::secure() ? 's' : '').'://';
        $host = $_SERVER['HTTP_HOST'].str_replace('/index.php', '', $_SERVER['PHP_SELF']);
        $fullurl = $host.(Config::get('ikpanel-config.admin_panel_url') == '/' ? '' : Config::get('ikpanel-config.admin_panel_url')).$url;
        $fullurl = str_replace('//', '/', $fullurl);
 ...
 * @param  Collection|array  $collection  Collezione o array di oggetti
